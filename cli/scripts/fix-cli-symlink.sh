#!/usr/bin/env bash
#
# fix-cli-symlink.sh — repoint the global `9router` install at THIS clone.
#
# Why this exists:
#   When more than one 9router clone exists on a machine (e.g. ~/9router-fork
#   and ~/Downloads/9router-fork), the global `9router` command may resolve to
...
#
#   This re-points every global node_modules/9router symlink found on PATH at
#   the clone this script lives in, so `9router` always launches this checkout.
#
# Usage:
...
if [ ! -f "$CLI_DIR/cli.js" ]; then
  echo "error: $CLI_DIR/cli.js not found — run this from a 9router clone" >&2
  exit 1
fi
...
# Find every global node_modules/9router on the PATH's npm prefixes.
...
for npm_bin in $(command -v -a npm 2>/dev/null || true); do
  ...
  [ -z "$prefix" ] && continue
  link="$prefix/lib/node_modules/9router"
...
if [ "$changed" = "0" ]; then
  echo "nothing to do — global 9router already points at this clone"
elif [ "$CHECK_ONLY" = "1" ]; then
  echo "run without --check to repoint"
  exit 1
fi

# Show where the user-facing `9router` command now resolves.
if command -v 9router >/dev/null 2>&1; then
  bin="$(command -v 9router)"
  target="$(cd "$(dirname "$bin")" && readlink "$bin" 2>/dev/null || true)"
  echo "PATH 9router: $bin -> ${target:-<self>}"
fi
