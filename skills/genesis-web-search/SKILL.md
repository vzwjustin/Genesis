...
description: Web search via Genesis /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity. Use when the user wants to search the web, look up information, find articles, or query a search engine.
---

# Genesis — Web Search

Requires `NINEROUTER_URL` (and `NINEROUTER_KEY` if auth enabled). See https://raw.githubusercontent.com/decolua/genesis/refs/heads/master/skills/genesis/SKILL.md for setup.
...
curl -X POST $NINEROUTER_URL/v1/search \
  ...
  -d '{"model":"tavily","query":"Genesis open source","max_results":5}'
```
