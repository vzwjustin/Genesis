...
const hasGenesisConfig = (config) => {
  ...
  if (providers["genesis"]) return true;
...
export async function GET(request) {
  ...
  const hasGenesis = hasGenesisConfig(config);
...
export async function POST(request) {
  ...
  try {
    ...
    env.JCODE_GENESIS_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile genesis",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE(request) {
  ...
  try {
    ...
    delete config.providers["genesis"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_GENESIS_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "genesis configuration removed from jcode",
    });
  } catch (error) {
