...
Combos are **custom fallback chains** that you create in the dashboard. Instead of using a single model, you define a sequence of models that Genesis tries in order.
...
Genesis automatically tries each model in sequence until one succeeds.
...
When limit reached, Genesis skips paid models and uses free tier only.
