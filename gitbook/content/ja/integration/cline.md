...
GenesisをCline VSCode拡張機能と統合し、AIリクエストをGenesisのインテリジェントルーティングシステム経由でルーティングします。
...
**手順:**
1. **Base URL** フィールドにGenesisエンドポイントを入力
2. 末尾に `/v1` を必ず含める
...
3. Genesis経由でAIによる説明を取得

### ファイル操作

1. Clineにファイルの作成、変更、削除を依頼
2. ClineはGenesisを使用してコンテキストを理解し変更を加える
3. 受け入れる前に変更を確認
...
2. Genesisインスタンスが動作中で正常か確認
3. VSCodeウィンドウをリロードしてみる (Cmd/Ctrl + Shift + P → 「Reload Window」)
4. エラーについてGenesisログを確認

## 高度な設定

### クラウドエンドポイントを使用

localhostの代わりにGenesisクラウドエンドポイントを使用:

1. Cline設定で、Base URLを設定: `https://genesis.com`
2. GenesisクラウドダッシュボードでAPIキーが設定されていることを確認
...
Genesisは以下に基づいて最適な利用可能なプロバイダーにリクエストを自動的にルーティング:
- モデル可用性
- プロバイダーヘルスステータス
- コスト最適化
- ロードバランシング
...
プロバイダーが失敗した場合、Genesisは自動的にダッシュボードで設定された代替プロバイダーにフォールバックします。

### 使用量トラッキング

Genesisダッシュボード経由でCline使用量をモニター:
- 総リクエスト数
- トークン使用量
- モデルごとのコスト
- プロバイダー分布
