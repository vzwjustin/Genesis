...
function verifyProtectedArray(arr, snap) {
  if (!snap) return true;
  const hasProtected = snap.some((s) => s != null);
  if (!Array.isArray(arr)) return !hasProtected;
  for (let i = 0; i < snap.length; i++) {
    if (snap[i] == null) continue;
    if (i >= arr.length || JSON.stringify(arr[i]) !== snap[i]) return false;
  }
  return true;
}

/** True when the client placed Anthropic cache_control breakpoints anywhere in the body. */
export function hasAnthropicCacheBreakpoints(body) {
  ...
  if (body.conversationState) {
    ...
    if (Array.isArray(history) && history.some((item) => itemHasCacheControl(item))) return true;
    if (itemHasCacheControl(body.conversationState.currentMessage)) return true;
  }
  return false;
}
...
export function verifyCacheProtectedBody(body, snapshot) {
  ...
  if (!verifyProtectedArray(body.tools, snapshot.tools)) return false;
