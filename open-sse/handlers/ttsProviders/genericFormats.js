...
import { responseToBase64, throwUpstreamError, ttsFetch } from "./_base.js";
...
async function hyperbolic({ baseUrl, apiKey, text, proxyOptions }) {
  const res = await ttsFetch(baseUrl, {
    method: "POST",
...
async function deepgram({ baseUrl, apiKey, text, modelId, proxyOptions }) {
  ...
  const res = await ttsFetch(url.toString(), {
    method: "POST",
...
async function huggingface({ baseUrl, apiKey, text, modelId, proxyOptions }) {
  ...
  const res = await ttsFetch(`${baseUrl}/${modelId}`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
