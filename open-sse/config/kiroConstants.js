 ...
 * CLIProxyAPIPlus reference implementation, scoped down to what genesis needs:
 *
 ...
 *
 * Kiro upstream does not advertise `-agentic` model IDs; they are a genesis
...
function readThinkingBudgetFromBody(body) {
  ...
  if (typeof oc === "string" && oc) {
    ...
    if (e === "auto") return KIRO_THINKING_BUDGET_DEFAULT;
    return effortToBudget(e) ?? KIRO_THINKING_BUDGET_DEFAULT;
  }
...
export function resolveKiroThinkingBudget(body, headers, model) {
  const fromBody = readThinkingBudgetFromBody(body);
...
/**
 * Detect whether a model id refers to a genesis synthetic agentic variant.
...
/**
 * Detect whether a model id is a genesis synthetic thinking variant
...
/**
 * Resolve a genesis model id to the real upstream Kiro model id, plus flags
